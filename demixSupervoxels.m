function [square_sAff, svMeans, svCells, voxelCounts, svColorMins, svColorMaxs, origIndex, duplications] = demixSupervoxels6(square_sAff, svMeans, svCells, voxelCounts, svColorMins, svColorMaxs, opts, origIndex)
...
%svMeansNormLUV                 = score(:, 1:min(size(score,2),size(svMeans, 2)));
[coeff,score,latent]           = pca(allColors, 'Economy', false);
...
origIndex(ToRemoveOriginal)        = 0;
mapDown                            = zeros(max(origIndex), 1);
uniqueIndices                      = setdiff(unique(origIndex), 0);
mapDown(uniqueIndices)             = 1:numel(uniqueIndices);
origIndex(origIndex~=0)            = mapDown(origIndex(origIndex~=0));
