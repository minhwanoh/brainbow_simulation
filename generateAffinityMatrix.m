...
cIdx                                         = 1;
pure                                         = intersect(graphData.smallSVs, find(graphData.svSizes>graphData.minSizeForPure & graphData.perims<graphData.maxPerim * sqrt(log(graphData.svSizes)))); % 30, 1e-10
Mdl                                          = KDTreeSearcher(graphData.colorsForDistal(pure, :));
MdlA                                         = KDTreeSearcher(graphData.colorsForDistal);
disp(1);
...
for kk = 1:numel(graphData.smallSVs)
  ...
  if ismember(kk1, pure)
    ...
    if numel(validcolorNeighbors)<graphData.minEdgeCountForProximal % 2
      [colorNeighbors2, cDist2]              = knnsearch(MdlA, graphData.colorsForDistal(kk1, :), 'K', graphData.minEdgeCountForProximal-numel(validcolorNeighbors));
      thisSV                                 = find(colorNeighbors2==kk1);
      toRemove                               = ismember(colorNeighbors2, [colorNeighbors thisSV]);
