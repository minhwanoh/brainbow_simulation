...
for kk = 1:cc
  ...
  if numel(neighbors)>1 & max(pdist(svMeansNormLUV(neighbors, :)))<opts.maxDistNormLUV & max(svColorMaxs(neighbors, :), [], 1)-min(svColorMins(neighbors, :), [] , 1)<opts.maxVoxColorDist*sqrt(log(sum(voxelCounts(neighbors))))
