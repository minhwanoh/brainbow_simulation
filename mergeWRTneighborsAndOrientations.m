...
for mm = 1:numel(smallerSVs)
  ...
  if oAff(kk, bestNeighbor6)>opts.minDotProduct & tt1<opts.maxColorDist & binsaff(kk, bestNeighbor6)>1/2 & maxVoxColorDist<opts.maxVoxColorDist*sqrt(log(sum(voxelCounts([kk bestNeighbor6]))))
    xx(idx)                                     = kk;
