function [index, graphData] = segmentImage_tmp(fileName, graphData)
load(fileName);
...
load(superVoxelOpts.dataset); bbVol = denoised; clear denoised; bbVol(bbVol<0)=0; for kk = 1:size(bbVol, 4); rawStack = bbVol(:,:,:,kk); rawStack = rawStack - min(rawStack(:)); rawStack = rawStack / max(rawStack(:)); bbVol(:,:,:,kk) = rawStack; end; clear rawStack;
graphData.svCells                   = svCells;
graphData.sig                       = 0;
opts_fkmeans.careful                = true;
opts_fkmeans.maxIter                = 10; % 20000;
...
warning off;
distortion                          = 1e60;
for kk = 1:graphData.kMeansIterCount
%  initialLabels                     = kmeans(topFewEigenvectors, graphData.opts_irbleigs.K,'MaxIter',2);
  initialLabels                     = kmeans(graphData.colorsForDistal(toKeep, :), graphData.opts_irbleigs.K,'MaxIter',20);
